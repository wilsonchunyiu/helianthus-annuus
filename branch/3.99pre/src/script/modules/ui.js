		...
		fn: function()
		{
			...
			$.addMain('選項', function()
			{
				if(!$('#an-settings').length)
				{
					...
					var oStructure = {};
					$.each(an.types, function(sType)
					{
						oStructure[sType] = {};
						$.each(an.pages, function(sPage)
						{
					...
					$.each(oStructure, function(sType)
					{
						...
						jContainer.append($.format('<li><a href="javascript:" rel="an-settings-panel-{0}">{1}</a></li>', sType, an.types[sType]));

						sHTML += $.format('<fieldset id="an-settings-panel-{0}"><legend>{1}</legend>', sType, an.types[sType]);
						$.each(this, function(sPage)
						{
							if($.isEmptyObject(this)) return;

							var aGroup = [];
							if(sPage in an.pages) aGroup.push(an.pages[sPage].desc);
							else $.each(an.pages, function(s)
							{
								if(s < 65534 && s & sPage) aGroup.push(this.desc);
							});
							sHTML += $.format('<h4><span>{0}</span><hr /></h4>', aGroup.join('、'));
							...
							$.each(this, function(sMixed)
							{
								...
								var sSwitchId = $.format('an-settings-switch-{0}{1}{2}', sPage, sSep, sMixed);
								var sChecked = this.comp ? 'checked="checked"' : '';
								var sSavable = (this.comp || bDisabled) ? 'disabled="disabled"' : 'class="an-settings-switch"';
								sHTML += $.format('<input type="checkbox" id="{0}" {1} {2} />', sSwitchId, sChecked, sSavable);
								sHTML += $.format('<label for="{0}">{1}</label></dt>', sSwitchId, this.desc);
								...
								if(this.options)
								{
									$.each(this.options, function(sName)
									{
										...
										var sOptionId = $.format('an-settings-option-{0}{1}{2}', sPage, sSep, sName);
										var sSavable = bDisabled ? 'disabled="disabled"' : 'class="an-settings-option"';

										if(this.type == 'checkbox')
										{
											sHTML += $.format('<input type="checkbox" id="{0}" {1} /><label for="{2}">{3}</label>', sOptionId, sSavable, sOptionId, this.desc);
										}
										else if(this.type == 'text')
										{
											sHTML += $.format('<label for="{0}">{1}: </label><input type="text" id="{2}" {3} />', sOptionId, this.desc, sOptionId, sSavable);
										}
										else if(this.type == 'select')
										{
											sHTML += $.format('{0}: <select id="{1}" {2}>', this.desc, sOptionId, sSavable);
											$.each(this.choices, function(i, choice)
											{
												sHTML += $.format('<option value="{0}">{0}</option>', choice);
											});
											sHTML += '</select>';
										}
					...
					jFieldsets.each(function()
					{
						...
						{
							$($.format('[rel={0}]', jThis.attr('id'))).parent().add(jThis).remove();
						}
					});
	...
	queue: [{
		...
		fn: function()
		{
			...
			$.addMain('評測', function()
			{
				...
				var aBg = [$.options('sMainBgColor'), $.options('sSecBgColor')];
	...
	queue: [{
		...
		fn: function()
		{
			...
			$.log = function(sLog)
			{
				...
				if($.options('bAutoShowLog')) jLog.fadeIn('fast');
				...
				$($.format('<li>{0}:{1} {2}</li>', hour, min, sLog)).prependTo(jLogContainer).slideDown('slow');

				jLogContainer.children(':gt(50)').remove();
			};

			if($.options('bShowDetailLog')) $.log2 = $.log;
	...
	queue: [{
		...
		fn: function()
		{
			var getMod = function()
			{
				...
				', $.options('bAutoShowLinks') ? '' : 'display: none;');
	...
	queue: [{
		...
		fn: function()
		{
			var getMod = function()
			{
				...
				',
				$.options('bAutoShowInfo') ? '' : 'display: none'
				);
...
'629944a0-a4f2-493c-8c8d-e1261a9264f9':
{
	...
	queue: [{
		...
		fn: function()
		{
			...
			.an-mod:hover { {1} } \
			',
			$.cssText('opacity', $.options('nUIOpacity') / 10),
			$.cssText('opacity', 1)
			);
		}
	}]
