...
'8be1ac06-030a-42d4-a8f4-f2b7f4881300':
{
	...
	queue: [{
		fn: function()
		{
			...
			var
			styleNo = this.styleNo = $.inArray($.options('quoteStyle'), this.options.quoteStyle.choices),
			jButton;
	...
	},
	{
		...
		fn: function()
		{
			var level = $.options('quoteMaskLevel') - (this.styleNo === 0 ? 1 : 2);
			if(level < 0) return;
...
'cb1917f9-4053-40b1-870d-e0e2c6a90b39':
{
	...
	queue: [{
		fn: function()
		{
			...
			var
			hideMode = $.inArray($.options('sQRHideMethod'), this.options.sQRHideMethod.choices),
			jQR = $('#newmessage'),
			...
			',
			$.cssText('opacity', $.options('nQROpacity') / 10),
			nWidth,
