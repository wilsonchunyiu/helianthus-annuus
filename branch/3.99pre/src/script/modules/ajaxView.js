		...
		fn: function()
		{
			var refreshTopics = function(nPage)
			{
				...
				$.getDoc($.search({ page: nPage }), function(jNewDoc)
				{
					...
					if(nPage == $.options('nNumOfTopicPage'))
					{
			...
			var tRefresh;
			var bAutoRefresh = $.options('bAutoRefresh_T');
			var nInterval = $.options('nRefreshInterval_T');
			if(nInterval < 30) nInterval = 30;

			if(bAutoRefresh) setNextRefresh();
			if($.options('bAddToggleAutoBtn_T')) $.run('addButton', '切換自動更新', function()
			{
				if(bAutoRefresh)
				{
					clearTimeout(tRefresh);
			...
			if($.options('bAddGetBtn_T')) {
				$.prioritize(4, 1, function()
				{
