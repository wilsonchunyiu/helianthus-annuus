...
if(an.storageMode === 'DOM') {
	...
	an.__lso = $('<div />', { id: 'an-lso' }).appendTo('#an').toFlash('http://helianthus-annuus.googlecode.com/svn/other/lso.swf' + ($.browser.msie ? '?' + $.time() : ''))[0];

	$.timeout('init', 250, function()
	{
		if(typeof an.__lso.get === 'function') {
			$d.an();
		}
		else {
			$.timeout('init', 50);
